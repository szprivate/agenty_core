...
- node clusters (required structure):
    - (none resolved)
- optional roles: GrokImageEditNode, SaveImage, LoadImage, LumaReferenceNode, MarkdownNote, ByteDanceSeedreamNodeV2, GeminiImage2Node, OpenAIGPTImageNodeV2, RecraftStyleV3InfiniteStyleLibrary, RegexReplace, ResizeAndPadImage, BatchImagesNode
...
## API / Partner Nodes - Image Edit / Seedream  (`api_partner_nodes_image_edit__seedream`)  -  5 workflow(s)  -  source: mixed
- execution: api (API nodes: ByteDanceSeedreamLayerSeparationNode, ByteDanceSeedreamNode, ByteDanceSeedreamNodeV2)
- when to use: Use to edit an existing image using Seedream.
- example request: "build an image workflow using Seedream"
- description: API / cloud image editing via Seedream 5.0 lite. 2 image inputs -> 1 image output. Processes and generates content using ComfyUI workflows. | Edit images with precision using Seedream 5.0 Pro, targeting specific regions while preserving lighting, depth, and texture. The workflow takes 1 input image and produces 2 output images through an optional Painter node and preview. Ideal for product photography edits, portrait consistency adjustments, and creating structured layouts with multilingual text. | Multi-modal AI model for text-to-image and image editing. Generate 2K images in under 2 seconds with natural language control. | Split any image into independent, editable layers - subject, product, background, and text - with a single text instruction using Seedream 5.0 Pro. A single poster can yield 10+ layers, with design-grade text separation supporting multilingual headlines and CJK fonts. Ideal for brand design, e-commerce asset creation, and layout iteration. | Upload an image of your product and background. Composite them and seamlessly relight and fuse together using Seedream 4.5.
- member workflows:
...
## API / Partner Nodes - Image Edit / Flux 2 Klein  (`api_partner_nodes_image_edit__flux_2_klein`)  -  1 workflow(s)  -  source: official
- execution: api (API nodes: ByteDanceSeedreamNode, GeminiImage2Node, GrokImageEditNode, OpenAIGPTImage1)
...
- description: upload one input image and select multiple editing models. generate side-by-side outputs to directly compare model effects and quality.
- member workflows:
...
- paired/multiple required: SaveImage x7, CLIPLoader x3, CLIPTextEncode x3, ReferenceLatent x3, UNETLoader x3, VAEDecode x3, VAEEncode x3, VAELoader x3, EmptyFlux2LatentImage x2, Flux2Scheduler x2, FluxKontextMultiReferenceLatentMethod x2, ImageScaleToTotalPixels x2, KSamplerSelect x2, MarkdownNote x2, RandomNoise x2, SamplerCustomAdvanced x2, TextEncodeQwenImageEditPlus x2
...
- node clusters (required structure):
    - (none resolved)
- optional roles: Note, CLIPTextEncode, MarkdownNote, CheckpointLoaderSimple, KSamplerAdvanced, BasicScheduler, BetaSamplingScheduler, CFGGuider, CFGNorm, CLIPLoader, ChromaRadianceOptions, ConditioningZeroOut
- unresolved nodes: MarkdownNote, Note, PrimitiveNode
...
- optional roles: MarkdownNote, ConditioningZeroOut, LoraLoaderModelOnly, ResolutionSelector, SaveImage, SaveImageAdvanced
- unresolved nodes: MarkdownNote
...
# Image Edit  (`image_edit`)  -  54 workflow(s), 14 model(s)

## Image Edit / Qwen Image  (`image_edit__qwen_image`)  -  17 workflow(s)  -  source: mixed
- execution: local
- when to use: Use to edit an existing image using Qwen Image.
- example request: "build an image workflow using Qwen Image"
- description: Advanced image editing with multi-image support, improved consistency, and ControlNet integration. | Decompose an image into editable RGBA layers for high-fidelity recolor, replace, resize, and reposition workflows using Qwen-Image-Layered. | Decomposes an image into variable-resolution RGBA layers for independent editing using Qwen-Image-Layered. | Edit images with Qwen Image Edit 2511 Int8, using a quantized model for faster inference while maintaining high quality. Input one image and receive one edited output. Ideal for character-consistent portrait editing, multi-person group photo fusion, and industrial design modifications. | Edit images with precise bilingual text editing and dual semantic/appearance editing capabilities using Qwen-Image-Edit's 20B MMDiT model. | Edits images from text instructions using Qwen-Image-Edit-2509 with optional Lightning LoRA for few-step sampling. | Edits images via text instructions using Qwen-Image-Edit-2511 with improved character consistency and integrated LoRA. | Image Edit blueprint | Input an illustration and generate a hyper realistic version using Qwen Image Edit 2509. | Local image editing via QWEN-Image-Edit-2511-Lightning. Up to 3 images (including optional depth/canny control inputs) -> 1 edited image output. Supports text-guided edits with optional structural control. | Relight images using Qwen-Image-Edit with LoRA support. | Replace materials in objects (e.g., furniture) by combining reference images with Qwen-Image-Edit-2511. | Upload a portrait of your character and a reference lighting image. Generate an image with reference lighting applied and character consistency. | Upload an image and specify a subject. Generate an inflated version of that subject using the INFL8 LoRA for Qwen Image Edit. | Upload an image of your scene and generate multiple views of your input scene with 1 click. | Upload your target image and a reference lighting image. Relights your target image.
...
- optional roles: ConditioningZeroOut, LoadImage, MarkdownNote, SaveImage, FluxKVCache
- unresolved nodes: MarkdownNote
...
- description: Generate a talking-head video from a single photograph using HeyGen's Avatar IV engine, which produces a fully animated video with lip-synced speech, facial expressions, head movements, and automatic gestures. Ideal for creating digital avatars for presentations, virtual characters for interactive content, and personalized video messages from a single image.
- member workflows:
    - api_heygen_talking_photo
- node clusters (required structure):
    - inputs: LoadImage
    - output: SaveVideo
    - other operations: HeyGenTalkingPhotoNode

## API / Partner Nodes - Text to Video / Flux  (`api_partner_nodes_text_to_video__flux`)  -  1 workflow(s)  -  source: official
- execution: api (API nodes: Flux3TextToVideoNode)
- when to use: Use to generate a video from a text prompt using Flux.
- example request: "build a video workflow using Flux"
- description: Generate full videos from a single text prompt with Flux.3, which expands natural language into complete scenes with synchronized audio, no reference images or complex settings needed. Outputs 1 video in 720p at 24fps, 5 to 20 seconds long, supporting any aspect ratio from 9:16 to 21:9 with built-in sound design. Ideal for quick social media clips, ad mockups, and concept films requiring realistic motion and cinematic style.
- member workflows:
    - api_bfl_flux3_t2v
- node clusters (required structure):
    - output: SaveVideo
    - other operations: Flux3TextToVideoNode
...
## Image to Video / WAN 2.2  (`image_to_video__wan_2_2`)  -  6 workflow(s)  -  source: mixed
- execution: local
...
- paired/multiple required: CLIPTextEncode x2
- optional roles: KSampler, LatentConcat, MarkdownNote, WanSoundImageToVideoExtend, Note, AudioEncoderEncode, AudioEncoderLoader, ImageFromBatch, KSamplerAdvanced, LatentCut, LoadAudio, LoadImage
- unresolved nodes: MarkdownNote, Note
...
- example request: "build a video workflow using Anima"
- description: Generate a new video by feeding MiniMax H3 a reference image, up to nine images, three videos, and three audio clips alongside a text prompt, letting the model match character identity, motion, camera, voice, or style from those sources while producing native stereo audio in a single pass. Output is a single MP4 video at up to 2K resolution and 24fps for roughly 15 seconds. Ideal for character-consistent animation, style-transfer video generation, and audio-synced content creation. | Turn a starting image into a short, high-quality video clip with MiniMax H3, a unified multimodal model that generates visuals and synchronized audio in a single pass. The workflow takes 1 input image and produces 1 video output, with optional text or reference media to steer motion, style, or sound. Ideal for rapid concept visualization, character-driven animation tests, and creating shareable social media clips with built-in sound design.
- member workflows:
...
- execution: api (API nodes: TopazVideoEnhanceV2)
...
- node clusters (required structure):
    - inputs: LoadVideo
    - output: SaveVideo
    - other operations: TopazVideoEnhanceV2
...
- node clusters (required structure):
    - model loading: CheckpointLoaderSimple, LTXVAudioVAELoader, LoraLoaderModelOnly
    - conditioning: CLIPTextEncode (x2), LTXAVTextEncoderLoader, LTXVConditioning
    - latent / canvas: LTXVEmptyLatentAudio
    - decoding: LTXVAudioVAEDecode, VAEDecodeTiled
    - other operations: CreateVideo, EmptyLTXVLatentVideo, GetImageSize, GetVideoComponents, LTXVAddGuide, LTXVConcatAVLatent, LTXVCropGuides, LTXVImgToVideoInplace, LTXVSeparateAVLatent, MarkdownNote, PrimitiveInt
- paired/multiple required: CLIPTextEncode x2
...
- node clusters (required structure):
    ...
    - conditioning: CLIPTextEncode (x2)
    - decoding: VAEDecode
    - other operations: CreateVideo, GetVideoComponents, ModelSamplingSD3, Wan22FunControlToVideo
- paired/multiple required: CLIPTextEncode x2
- optional roles: KSamplerAdvanced, MarkdownNote, Canny, KSampler, PreviewImage, SaveVideo, VHS_VideoCombine
- unresolved nodes: MarkdownNote
...
## Video to Video / Generic  (`video_to_video__generic`)  -  1 workflow(s)  -  source: official
- execution: local
- when to use: Use to increase a video's frame rate via interpolation.
- example request: "build a video workflow"
- description: Upload a video and select either RIFE or FILM frame interpolation model to generate smoother, higher frame rate video output.
- member workflows:
    - utility_video_frame_interpolation
- node clusters (required structure):
    - inputs: LoadVideo
    - model loading: FrameInterpolationModelLoader
    - output: SaveVideo
...
## API / Partner Nodes - Upscale / Z-Image  (`api_partner_nodes_upscale__z_image`)  -  3 workflow(s)  -  source: mixed
- execution: api (API nodes: TopazImageEnhance, TopazImageEnhanceV2)
- when to use: Use to upscale / enhance an image using Topaz, Z-Image.
- example request: "build an image workflow using Topaz"
- description: API image enhancement/upscaling via Topaz Reimagine. 1 image -> 1 enhanced image output. Applies face enhancement and detail restoration for professional results. | Enhance AI-generated images with Bloom 2, a creative upscaling model that adds new detail, texture, and visual elements while offering a Creativity control from 1-9 for subtle refinement or bold reinterpretation. It accepts 1 input image and produces 1 enhanced output, with optional Face Recovery, Color Preservation, and grain settings for fine-tuned results. Ideal for improving AI artwork and illustrations, adding realism to CG renders, and refining facial detail in generated portraits. | Upscale and enhance a single image using Topaz's Wonder 3.5 generative model, which adds fine detail, reduces noise, and sharpens text and structured graphics while offering Low, Medium, and High enhancement strengths. The workflow takes 1 input image and produces 1 enhanced output alongside a side-by-side comparison view. Ideal for restoring old or compressed photos, improving product shots and marketing visuals, and preparing low-resolution images for print or large-format display.
- member workflows:
...
- node clusters (required structure):
    - inputs: LoadImage
- optional roles: ImageCompare, MarkdownNote, SaveImage, SaveImageAdvanced, TopazImageEnhance, TopazImageEnhanceV2
- unresolved nodes: MarkdownNote
...
# Image Edit with ControlNet  (`image_edit_with_controlnet`)  -  16 workflow(s), 7 model(s)
...
- example request: "build a video workflow"
- description: Local, simple image upscaling via specified ESRGAN model. 1 image -> 1 upscaled image output. Supports various models. | Upload a video to upscale it using a fast GAN model. The output resolution is determined by your chosen model (e.g., 2x or 4x). For best results, upscale first, then resize to your target resolution. | Upload an image and select an interpolation method. Upscale the image using traditional algorithms for faster processing with minimal detail changes. | Upscale and restore video footage using SeedVR2 3B Int8, a one-step diffusion model that enhances resolution while maintaining temporal consistency across frames. This workflow takes one degraded video as input and outputs a single high-resolution, restored video. Ideal for restoring old or degraded footage, upscaling low-resolution videos, and enhancing archival media. | Upscale images using SeedVR2 3B Int8, a one-step diffusion-based video restoration model that produces high-quality results with improved temporal consistency. Ideal for restoring degraded footage, upscaling low-resolution videos, and enhancing archival media. | Upscale images using SeedVR2 7B Int8, a one-step diffusion model that enhances resolution through adversarial training and adaptive window attention. Input 1 image and receive 1 upscaled output with improved detail and clarity. Ideal for restoring low-resolution photos, preparing assets for print, or sharpening lightly degraded digital images. | Upscales video to 4× resolution using a GAN-based upscaling model.
- member workflows:
    - upscale_using_model
...
- node clusters (required structure):
    - (none resolved)
- optional roles: MarkdownNote, CreateVideo, GetVideoComponents, ImageCompare, ImageScaleBy, ImageUpscaleWithModel, JoinImageWithAlpha, KSampler, LoadImage, LoadVideo, ResizeImageMaskNode, SaveImage
- unresolved nodes: MarkdownNote
...
## Upscale / Z-Image  (`upscale__z_image`)  -  2 workflow(s)  -  source: official
- execution: local
- when to use: Use to upscale / enhance an image using Z-Image.
- example request: "build an image workflow using Z-Image"
- description: Upload an image to upscale it to 2K resolution using the Z-Image-Turbo model. | Upscales images to higher resolution using Z-Image-Turbo.
- member workflows:
...
- example request: "build a video workflow using WAN 2.2"
- description: Creates a smooth video using 6 key frames.It auto fills in the motion between frames and stitches the segments together seamlessly.
- member workflows:
...
- paired/multiple required: CLIPTextEncode x2, KSamplerAdvanced x2, ModelSamplingSD3 x2, UNETLoader x2, LoadImage x2
- optional roles: LoraLoaderModelOnly, Note, BatchImagesNode, GetVideoComponents, MarkdownNote, SaveVideo, VHS_VideoCombine
- unresolved nodes: MarkdownNote, Note
...
# Video Tools  (`video_tools`)  -  5 workflow(s), 2 model(s)
...
# API / Partner Nodes - Character  (`api_partner_nodes_character`)  -  4 workflow(s), 2 model(s)

## API / Partner Nodes - Character / Generic  (`api_partner_nodes_character__generic`)  -  2 workflow(s)  -  source: official
- execution: api (API nodes: GeminiImage2Node)
- when to use: Use to relight an image.
- example request: "build an image workflow"
- description: Upload a portrait of your character and create multiple cinematic lighting conditions while maintaining character consistency. | Upload your character and generate 360 turnaround views for full body and close ups.
- member workflows:
...
- paired/multiple required: GeminiImage2Node x2, SaveImage x3
- optional roles: ImageStitch
...
# API / Partner Nodes - Inpaint / Outpaint  (`api_partner_nodes_inpaint_outpaint`)  -  4 workflow(s), 2 model(s)
...
# API / Partner Nodes - Image Edit with ControlNet  (`api_partner_nodes_image_edit_with_controlnet`)  -  2 workflow(s), 1 model(s)
